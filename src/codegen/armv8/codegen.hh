    ...
    if (src1.p_typ == Param::Reg && src2.p_typ == Param::Reg) // both reg
        asm_insts.push_back(AsmCode(_i_typ,
            {   Param(r),
                ...
                src2},
            indent));
    else if (src1.p_typ == Param::Reg) // src1 is reg, src2 is ctv
    {
...
void AddAsmCodeFromLLIR(vector<AsmCode> &asm_insts, map<int32_t, REGs> &AllocationResult, Inst *instPtr, int indent)
{
    ...
    Case (LLIR_BIN, bin_inst, instPtr)
    {
        ...
        if (src1.p_typ == Param::Imm_int && src2.p_typ == Param::Reg)
            std::swap(src1, src2);
        if (src1.p_typ == Param::Imm_int && src2.p_typ == Param::Imm_int && src1.val.i < src2.val.i)
            std::swap(src1, src2);
...
void GenerateAssembly(const string &asmfile, const CompUnit &ir)
{
    ...
    for (auto &&funcPtr : ir.functions)
    {
        ...
        for (auto &&bbPtr : funcPtr->all_blocks)
        {
            ...
            {
                ++indent;
                AddAsmCodeFromLLIR(asm_insts, funcPtr->AllocationResult, instPtr, indent);
                --indent;
            }
        }
    }
